...
CUresult cuDevicePrimaryCtxRetain(CUcontext *pctx, CUdevice dev){
    ...
    unsigned int nvmlpid = (unsigned int)getpid();
    ...
    if (charge == 0 && pidfound == 1 &&
        context_accounting[dev].retain_count == 0) {
        nvmlReturn_t result =
            get_used_gpu_memory_by_pid(nvmlpid, dev, &before);
    ...
    if (measure_context) {
        ...
        for (attempt = 0; attempt < 10; attempt++) {
            if (get_used_gpu_memory_by_pid(nvmlpid, dev, &after) ==
                    NVML_SUCCESS &&
                after > before) {
